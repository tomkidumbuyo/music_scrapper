...
# scrapre lyrics from the site (genius) the search
# the song on youtube and download the said song.
def read_lyrics(page_url):
    ...
    if len(lyrics) > 0:
        ...
        output_message = (tittle + "\n" + artist)
        if(featuring){
            output_message += " ft. " + featuring
        }
        output_message += ("Producer: " + producer)
...
# this function gets all links from a webpage and adds them to 
# @all_links array. Duplicate links not added;
